...
//商品列表相关的逻辑
const useShopProductEffect = (id) => {
  const products = reactive({ products: [] });

  const getProducts = async (tab) => {
    const res = await getShopProductList(id, tab);
...
const useCartEffect = () => {
  ...
  const { cartList } = reactive({cartList:store.state})
...
export default {
  setup() {
    ...
    const { currentTab, handleTab } = useCurrentTabEffect(id);

    const { products, getProducts } = useShopProductEffect(id);
